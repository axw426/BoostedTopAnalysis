...
void LeptonEfficiencyChecker::init() { 
  ...
  if( pHistogramFactory!=0 )
      ...
      if (!(pTree->cd( "/" + name() + "/"))) 
	{
      ...
      _tLinkerData = new TTree("LinkerData", "Tree containing final reconstructed masses");
    }
}
...
void LeptonEfficiencyChecker::processEvent( LCEvent * evt ) { 
  ...
  nLeptons=_MCCol->getNumberOfElements();
  int nCorrect=0;
  ...
  for (int i=0; i<nLeptons; i++)
    {
      ...
      mc= dynamic_cast<MCParticle*>( _MCCol->getElementAt(i) );

      //check if lepton is real or fake
      if(IsCorrectLepton(mc)==true)
	{

	  nCorrect++;
	  LCRelationNavigator* relationNavigatorPFOMC = new LCRelationNavigator( LinkerCol );
	  EVENT::LCObjectVec relobjMC = relationNavigatorPFOMC->getRelatedFromObjects(mc);
	   for(int j=0; j<relobjMC.size(); j++)
	    {
	      ReconstructedParticle* linkedRP=dynamic_cast <ReconstructedParticle*>(relobjMC[j]);
	      _vLinkedRPID.push_back(linkedRP->getType());
	      _vLinkedRPEnergy.push_back(linkedRP->getEnergy());
	      _vNRecoPerMC.push_back(relobjMC.size());
...
void LeptonEfficiencyChecker::end() {
  ...
  AddBranch(_tLinkerData, "NRecoPerMC",_vNRecoPerMC );
  AddBranch(_tLinkerData, "LinkedRPID",_vLinkedRPID );
  AddBranch(_tLinkerData, "LinkedRPEnergy",_vLinkedRPEnergy );
