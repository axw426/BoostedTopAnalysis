#include "LeptonOptimizationProcessorB.h"
...
LeptonOptimizationProcessorB aLeptonOptimizationProcessorB ;

LeptonOptimizationProcessorB::LeptonOptimizationProcessorB()
  : Processor("LeptonOptimizationProcessorB") {

  // ProcessorB description
  _description = "Isolated Lepton Finder ProcessorB-> aw jet isolation performs box vuts on min z and xt" ;
...
void LeptonOptimizationProcessorB::init() { 
...
void LeptonOptimizationProcessorB::processRunHeader( LCRunHeader* run) { 
} 

void LeptonOptimizationProcessorB::processEvent( LCEvent * evt ) { 
  ...
  try{
    ...
    {
      streamlog_out(WARNING) << _inputLepCollection << " collection not available, exiting IsolatedLeptonProcessorB..." << std::endl;
      _LepCol = NULL;
      return;
    }
  ...
  try{
    ...
    {
      streamlog_out(WARNING) << _MCColName << " collection not available, exiting IsolatedLeptonProcessorB..." << std::endl;
      _MCCol = NULL;
      return;
    }
  ...
  try{
    ...
    {
      streamlog_out(WARNING) << _LinkerColName << " collection not available, exiting IsolatedLeptonProcessorB..." << std::endl;
      LinkerCol = NULL;
      return;
    }
  ...
  for (int i=0; i<nLeptons; i++)
      ...
      if(d0 < lowestimpactparameter)
	{
      ...
      _vHighestJetZ.push_back(0);
    }
  if(nCorrectLeptons>1){std::cout<<"duplicate real leptons found"<<std::endl;}
  
  if(nLeptons>0)
    {
      _vHighestEnergyYN.at(bestenergyposition)=1;
      _vLowestImpactParameterYN.at(bestimpactparameterposition)=1;
      _vHighestJetZ.at(bestimpactparameterposition)=1;
    }
...
void LeptonOptimizationProcessorB::check( LCEvent * evt ) { 
}

void LeptonOptimizationProcessorB::end() {

  AddBranch(_tData, "nCorrectLeptons",_vnCorrectLeptons );
  ...
  AddBranch(_tData, "HighestJetZ",_vHighestJetZ );
...
bool LeptonOptimizationProcessorB::IsCorrectLepton(ReconstructedParticle *lepton)
{
  passedtest =false;
...
bool LeptonOptimizationProcessorB::IsCorrectLepton(MCParticle *mcp)
{
  passedtest =false;
...
void LeptonOptimizationProcessorB::AddBranch( TTree* input_tree, std::string title, std::vector<float> data )
{
...
float LeptonOptimizationProcessorB::getConeEnergy( ReconstructedParticle* pfo ) {
	float coneE = 0;
